...
| 9 | 46 self-grown + 14 published segments across the 8.64 µm batch | uniformly silent |
...
| 11 | paired 9.36 µm probe (§15 eval) | fragments-only vs wide model, both sim-9.36 fine-tuned, on validated published 1203 segments | P2: both silent on validated 1203 surfaces with passing sanity letterforms — 1203 @ 9.36 µm is silent under budget-adequate models |
| 12 | Scroll 5 end-to-end validation (addendum) | full pipeline + both models on a real, community-read scroll held out from training | **positive control passed** — co-locates with the community prediction (r → 0.40 at 4 mm blocks, flipped alignments ≈ 0); wide model ~2× more sensitive |
| 13 | multi-energy contrast control (addendum) | PHerc 51 Frag 6 at 53/70/88 keV, PPM-sampled labels | families are object-borne: d = +0.031/−0.029/−0.029 across energies — beam energy excluded |
| 14 | image-space degradation ladder (addendum, +14b) | blur readable Scroll 5 toward the eligible batch's texture statistics | reading survives to gradstd 2.96, collapses at 1.80; the eligible batch sits at 5.8 — image-space acquisition excluded with ~2× margin |
| 15 | family-based training selection A/B (addendum) | same-family vs cross-family training pairs, two held-out targets, matched budget | **refuted** — one training set won both targets (Δ +0.154 / +0.300 AUC); label density (0.12–0.18 vs 0.06–0.09) is the better predictor, confounded with family in this corpus |
